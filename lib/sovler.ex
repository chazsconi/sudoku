defmodule Solver do
  ...
  def solve_cells_recurse(%Grid{}=grid, opts \\ []) do
    case solve_cells(grid, opts) do
      {grid1, 0} -> grid1
      {grid1, _} -> solve_cells_recurse(grid1, opts)
    end
  end

  @doc "tries solves all cells"
  def solve_cells(%Grid{}=grid, opts \\ []) do
    ...
    Enum.reduce(Grid.all_cells, {grid, 0},
      fn(cell, {grid1, change_count}) ->
        ...
        case eliminate_peers_values_from_cell(grid1, cell) do
